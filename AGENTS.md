...
### 图片加载
- `exercises.json` 含 52 个图片引用，当前对应 48 个唯一 JPG 文件，打包到 `ExerciseImages/` bundle 目录
...
- Repository Protocols — `WorkoutRepository` / `ExerciseRepository` / `TemplateRepository` / `PersonalRecordRepository`
- `LocalExerciseDataService` — JSON 数据单例
- `ExerciseResourceValidator` — JSON/图片引用完整性校验
- `StatsCalculator` — 统计纯计算逻辑
- `PRService` — 个人记录评估
- `HapticManager` — 触觉反馈

## 自动测试
- `FitRockTests` — 业务单测、临时 SQLite 仓储测试、资源一致性测试
- `FitRockUITests` — 启动、Tab 导航、训练和动作库 UI 流程测试
- 本地编译测试：`xcodebuild build-for-testing -project FitRock.xcodeproj -scheme FitRock -destination 'generic/platform=iOS' CODE_SIGNING_ALLOWED=NO`

## 功能扩展 (v2026-06-28+)
- 动作历史详情：从动作详情进入，展示训练次数、总组数、PR、最近记录和相关肌肉
- 肌肉热力图：统计页使用 MuscleMap `BodyView.heatmap` 展示具体肌肉训练量，支持近7/30/90天/全部
- 4周训练计划：统计页生成 PPL/全身/增肌/减脂计划，支持手动完成/跳过/调整
- HealthKit：可选读取体重、心率、活动能量用于推荐；无授权时基于本地训练历史推荐
