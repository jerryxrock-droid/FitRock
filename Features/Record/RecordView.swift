...
struct SwipeableSetRow: View {
    ...
    @State private var weightDebounce: DispatchWorkItem?
    @State private var repsDebounce: DispatchWorkItem?
        ...
        HStack(spacing: Theme.Spacing.sm) {
            ...
            TextField(valuePlaceholder, text: $weightText)
                ...
                .onChange(of: weightText) { _ in
                    ...
                    let task = DispatchWorkItem { [weightText, repsText] in
                        let weight = unit == .duration ? Double(Int(weightText) ?? 0) : (Double(weightText) ?? 0)
